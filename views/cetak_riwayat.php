...
if (!isset($_SESSION['id_user'])) {
  echo "<div class='alert alert-danger'>Akses ditolak! Halaman ini hanya untuk user login.</div>";
  exit;
}
...
$q = mysqli_query($conn, "
  SELECT p.*, k.nama_kategori, l.nama_lokasi, t.isi_tanggapan, t.created_at AS tgl_tanggapan, a.nama_lengkap AS admin
  FROM pengaduan p
  LEFT JOIN kategori k ON p.id_kategori = k.id_kategori
  LEFT JOIN lokasi l ON p.id_lokasi = l.id_lokasi
  LEFT JOIN tanggapan t ON p.id_pengaduan = t.id_pengaduan
  LEFT JOIN users a ON t.id_admin = a.id_user
  WHERE p.id_pengaduan = $id AND p.id_user = $id_user
");

$data = mysqli_fetch_assoc($q);
if (!$data) die("<div class='alert alert-danger'>Data tidak ditemukan atau bukan milik Anda.</div>");
...
<head>
  <meta charset="UTF-8">
  <title>Laporan Pengaduan Saya</title>
...
<div class="print-page">
  ...
  <div class="laporan">
    <table class="table table-bordered">
      <tr><th width="30%">Pelapor</th><td><?= htmlspecialchars($_SESSION['nama_lengkap'] ?? 'User') ?></td></tr>
      <tr><th>Judul</th><td><?= htmlspecialchars($data['judul']) ?></td></tr>
      <tr><th>Kategori</th><td><?= htmlspecialchars($data['nama_kategori'] ?? '-') ?></td></tr>
      <tr><th>Lokasi</th><td><?= htmlspecialchars($data['nama_lokasi'] ?? '-') ?></td></tr>
